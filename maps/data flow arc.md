...
flowchart TD
    ...
    subgraph Stage5 ["5. 可视化呈现 (Visualize)"]
        APIServer[["🚀 FastAPI 接口<br/>backend/routers"]]:::process
        FrontendGraph[["🎨 Vue 图谱组件<br/>frontend/components/GraphView.vue"]]:::ui
    end

    %% 阶段 6: 关系弧深挖 (Arc Analysis) - NEW
    subgraph Stage6 ["6. 关系弧深挖 (Arc Analysis)"]
        NarrativeEngine[["🕸️ 叙事引擎 (Engine)<br/>backend/narrative_engine"]]:::process
        ArcState["📈 关系演化数据 (Arc)<br/>Trust/Romance/Conflict"]:::db
    end
    ...
    %% --- 新增 Arc 相关数据流 ---
    FrontendGraph --"1. 选中两实体/批量分析"--> APIServer
    APIServer --"2. 触发 Job (Analysis)"--> NarrativeEngine
    NarrativeEngine <-->|"3. 上下文推理"| LLMService
    NarrativeEngine <-->|"4. 读取章节摘要/存取状态"| SQLiteDB
    SQLiteDB --"5. 返回 Arc 历史数据"--> APIServer
```