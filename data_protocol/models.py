...
# --- Module Extensions (Vibe Coding Phase 3) ---

class ConceptStage(BaseModel):
    """模块 2: 渐进式世界观 - 概念阶段"""
    stage_name: str = Field(..., description="阶段名称: Unknown(未知), Rumor(传闻), Fact(事实), Truth(真相/核心)")
    description: str = Field(..., description="当前阶段对概念的理解")
    revealed_by: List[str] = Field(default_factory=list, description="揭示该阶段的关键事件或线索")

class ClueState(BaseModel):
    """模块 3: 契诃夫之枪 - 线索状态"""
    state: str = Field(..., description="状态: Setup(引入), Reminder(重提), Payoff(爆发/解决), Forgotten(遗忘)")
    chapter_index: int = Field(..., description="状态变更发生的章节索引")
    context: str = Field(..., description="上下文描述")

class FactionInfo(BaseModel):
    """模块 4: 派系热力图 - 势力信息"""
    faction_name: str = Field(..., description="势力名称")
    territory: List[str] = Field(default_factory=list, description="控制的区域/地点列表")
    enemies: List[str] = Field(default_factory=list, description="敌对势力列表")
    allies: List[str] = Field(default_factory=list, description="盟友列表")

# --- Extended Aggregations ---

class ExtendedAggregatedEntity(AggregatedEntity):
    """支持新模块的扩展实体"""
    # Module 2
    concept_evolution: List[ConceptStage] = Field(default_factory=list, description="概念演变历史")
    # Module 3
    clue_lifecycle: List[ClueState] = Field(default_factory=list, description="线索生命周期")
    # Module 4
    faction: Optional[str] = Field(None, description="所属势力 (若自身是组织，则为空或自指)")
